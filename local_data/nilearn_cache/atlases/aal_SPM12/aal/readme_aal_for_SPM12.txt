Anatomical Automatic Labeling (AAL) for SPM12
Date 25/08/2015
E-mail: aal.gin.brainatlas@gmail.com
...
in each hemisphere. This procedure was performed using dedicated software 
...
anatomical labeling of functional studies are proposed:
...
An interface with the Statistical Parametric Mapping (SPM12) package (Friston et 
al., 1995) is provided as a freeware to researchers of the neuroimaging 
community. We believe that this tool is an improvement for the macroscopic 
labeling of activated area as compared to labeling assessed using the 
Talairach atlas brain in which deformations are well known. However, this tool 
does not alleviate the need for more sophisticated labeling strategies based on 
anatomical or cytoarchitectonic probabilistic maps.

II Reference

If you need to reference this work, please use the following reference:
...
III.1 ROI_MNI_V4.nii (Neurological Orientation (R is R)). Each anatomical region is 
...
than the SPM12 distribution (/usr/local/soft/spm12/toolbox)

unix> cp aal_for_SPM12.tar.gz /usr/local/soft/spm12/toolbox
unix> cd /usr/local/soft/spm12/toolbox

IV.2 Expand the archive will create an aal directory
unix> tar -zxvf aal_for_SPM12.tar.gz

IV.3 Add this directory to your Matlab path
unix > setenv MATLABPATH ${MATLABPATH}:/usr/local/soft/spm12/toolbox/aal
unix > cp /usr/local/soft/spm12/toolbox/aal/atlas/AAL.nii /usr/local/soft/spm12/atlas
unix > cp /usr/local/soft/spm12/toolbox/aal/atlas/AAL.xml /usr/local/soft/spm12/atlas
...
V.2 First option: launch AAL from SPM12:
>> spm fmri
...
Then choose a labeling procedure as below from the instructions V.3.

V.2bis) An alternative is to launch AAL from the Matlab command window:
>> aal
Select the desired contrast, mask, probability and extent threshold like in the regular spm_result.
Then choose a labeling procedure as below from the instructions V.3.

V.3 Choose a labeling procedure. The 3 choices are explained and documented in 
...
V.4 For "Extended local maxima labeling" input the local maxima radius of the 
sphere in millimeters (default 10 mm).

V.5 Select the anatomical parcellation database:
In /usr/local/soft/spm12/toolbox/aal
The file ROI_MNI_V4.img

V.6 Results

V.6.1 Local maxima labeling
...
V.6.2 Extended local maxima labeling
...
VII Contact
Any comments could be sent to aal.gin.brainatlas@gmail.com
